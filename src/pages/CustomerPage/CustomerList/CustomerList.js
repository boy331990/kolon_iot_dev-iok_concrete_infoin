...
export const CustomerList = () => {
    ...
    const {loading, data, refetch} = useAxios({
        url: process.env.REACT_APP_API_GATEWAY + "/users?authorities=ROLE_CUSTOMER"
    });
    ...
    return (
        <>
            ...
            <div className={classes.content}>
                {loading && 'Loading...'}
                {data && <EnhancedTable title={"관리자"} rows={data.data.content} headCells={headCells} rowClick={handleClick} deleteClick={handleDelete}/>}
            </div>
        </>
    );
};